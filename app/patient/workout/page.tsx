...
import { Sun, Moon, PauseCircle, Check } from "lucide-react"

// --- COMPONENT: StreakWeek ---
function StreakWeek({ doneDays }: { doneDays?: number[] }) {
  // doneDays: indices (0..6) where 0 is 6 days ago, 6 is today
  ...
  const doneSet = new Set(doneDays ?? [6]) // default: only today done

  return (
    <div className="w-full max-w-5xl mx-auto mt-6">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-md font-semibold text-gray-700 dark:text-gray-200">Weekly Streak</h3>
        <div className="text-sm text-gray-500 dark:text-gray-400">1 week streak</div>
      </div>
        ...
        {days.map((d, idx) => {
          ...
          return (
            <div key={idx} className="flex flex-col items-center">
              <div
                className={`w-14 h-14 rounded-xl flex items-center justify-center transition-all duration-200 ${done ? 'bg-green-500 text-white shadow-lg' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700'}`}
              >
                {done ? <Check className="w-6 h-6" /> : <div className="text-sm font-medium">{dateNum}</div>}
              </div>
              <div className="text-xs mt-2 text-center text-gray-600 dark:text-gray-400">{short}</div>
            </div>
          )
        })}
      </div>
...
export default function WorkoutPage() {
  ...
  const [doneDays, setDoneDays] = useState<number[] | undefined>(undefined)
  ...
  function loadStreak(): StreakState {
    if (typeof window === "undefined") return { lastDate: null, streak: 0, history: [] }
    try {
      const raw = localStorage.getItem(STREAK_KEY)
      return raw ? JSON.parse(raw) : { lastDate: null, streak: 0, history: [] }
    } catch {
  ...
  function saveStreak(s: StreakState) {
    ...
    try { localStorage.setItem(STREAK_KEY, JSON.stringify(s)) } catch (e) { /* noop */ }
  }

  // on mount: award streak XP once per day and compute doneDays for the week UI
  useEffect(() => {
    const today = new Date()
    const todayStr = today.toISOString().slice(0, 10) // YYYY-MM-DD
    const s = loadStreak()
    // if already recorded for today, just compute doneDays
    if (s.history.includes(todayStr)) {
      // compute doneDays from history
      const daysArr = Array.from({ length: 7 }).map((_, i) => {
        const d = new Date()
        d.setDate(today.getDate() - (6 - i))
        return d.toISOString().slice(0, 10)
      })
      const indices = daysArr.map((d, idx) => (s.history.includes(d) ? idx : -1)).filter(i => i >= 0) as number[]
      setDoneDays(indices)
      return
    }

    // New visit for today — decide whether streak continues
    let newStreak = 2
    if (s.lastDate) {
      const last = new Date(s.lastDate)
      const diff = Math.floor((new Date(todayStr).getTime() - new Date(s.lastDate).getTime()) / (24 * 60 * 60 * 1000))
      if (diff === 1) {
    ...
    // award XP: 5 * n for nth-day streak
    try {
      addExp(5 * newStreak, 'streak:login', { streak: newStreak })
    } catch (e) {
      // ignore
    }

    const newHistory = [...s.history, todayStr].slice(-14) // keep recent history
    const newState: StreakState = { lastDate: todayStr, streak: newStreak, history: newHistory }
    saveStreak(newState)

    // compute doneDays array for the past 7 days
    const daysArr = Array.from({ length: 7 }).map((_, i) => {
      const d = new Date()
      d.setDate(today.getDate() - (6 - i))
      return d.toISOString().slice(0, 10)
    })
    const indices = daysArr.map((d, idx) => (newHistory.includes(d) ? idx : -1)).filter(i => i >= 0) as number[]
    setDoneDays(indices)
  }, [addExp])
  ...
  {/* Weekly streak UI below the canvas */}
  <StreakWeek doneDays={doneDays} />
